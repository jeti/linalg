...
import static io.jeti.linalg.matrix.TestConditions.getEqualityTolerance;
import static io.jeti.linalg.matrix.TestConditions.maxCols;
import static io.jeti.linalg.matrix.TestConditions.maxRows;
import static io.jeti.linalg.matrix.TestConditions.minCols;
import static io.jeti.linalg.matrix.TestConditions.minRows;
import static junit.framework.TestCase.assertTrue;

import io.jeti.linalg.matrix.interfaces.Mat;
import io.jeti.linalg.matrix.interfaces.Mat.Filler;
...
public class MatTest {
    ...
    public void size() throws Exception {
        forAllSizes((base, rows, cols) -> {
            ...
            for (Mat mat : mats) {
                assertTrue(mat.rows() == rows);
                assertTrue(mat.cols() == cols);
            }
        });
    }
    ...
    public void times() throws Exception {
        forAllSizes((base, rows, cols) -> {
            ...
            for (int inner = minCols; inner <= maxCols; inner++) {
     ...
     *         {@link io.jeti.linalg.matrix.interfaces.Mat} class to be tested.
     */
    private static Mat getInstance() {
        return new Matrix(1);
    }
     ...
     *         {@link MatFiller}.
     */
    private static <T extends Number> List<List<T>> generateList(int rows, int cols,
            MatFiller<T> filler) {
     ...
     * {@link io.jeti.linalg.matrix.interfaces.Mat} takes the value specified by
     * the given {@link Filler}, where equality is tested using the
     * {@link TestCase#assertEquals(double, double, double)} method with the
     * tolerance specified by the {@link TestConditions#getEqualityTolerance()}
     * method.
     */
    private static void assertEquals(Mat mat, Filler filler) throws Exception {
    ...
    /**
     * Ensure that each element of the specified
     * {@link io.jeti.linalg.matrix.interfaces.Mat} is the same as the value
     * returned by the given {@link Filler}, where sameness is tested using the
     * {@link TestCase#assertSame(Object, Object)} method.
     */
    private static <T extends Number> void assertSame(Mat mat, MatFiller<T> filler)
            throws Exception {
        for (int row = 0; row < mat.rows(); row++) {
            for (int col = 0; col < mat.cols(); col++) {
    ...
    private static interface TestElems {
        void test(Mat base, int rows, int cols) throws Exception;
    }

    private static void forAllSizes(TestElems testCase) throws Exception {
    ...
    private static int index(int row, int col, int rows) {
        return row + col * rows;
    }
}