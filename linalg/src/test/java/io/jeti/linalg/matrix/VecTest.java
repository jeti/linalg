...
import static io.jeti.linalg.matrix.TestConditions.elemMax;
import static io.jeti.linalg.matrix.TestConditions.elemMin;
...
import io.jeti.linalg.matrix.interfaces.Vec.Filler;
...
public class VecTest {
     ...
     *         {@link io.jeti.linalg.matrix.interfaces.Vec} class to be tested.
     */
    private static Vec getInstance() {
        return new Vector(1);
    }
     ...
     *         {@link ListFiller}.
     */
    private static <T extends Number> List<T> generateList(int elems, ListFiller<T> filler) {
     ...
     * {@link io.jeti.linalg.matrix.interfaces.Vec} takes the value specified by
     ...
     * method.
     */
    private static void assertEquals(Vec vec, Filler filler) throws Exception {
    ...
    /**
     * Ensure that each element of the specified
     * {@link io.jeti.linalg.matrix.interfaces.Vec} is the same as the value
     * returned by the given {@link Filler}, where sameness is tested using the
     * {@link TestCase#assertSame(Object, Object)} method.
     */
    private static <T extends Number> void assertSame(Vec vec, ListFiller<T> filler)
            throws Exception {
        for (int ind = 0; ind < vec.size(); ind++) {
    ...
    private static void forAllSizes(TestElems testCase) throws Exception {
        ...
        for (int elems = elemMin; elems <= elemMax; elems++) {
